...
const dayAliases = {
  ...
  "周五": 4, "星期五": 4, "礼拜五": 4,
  "周六": 5, "星期六": 5, "礼拜六": 5,
  "周日": 6, "星期日": 6, "星期天": 6, "礼拜天": 6, "礼拜日": 6, "周天": 6
};
...
async function startVoiceRecording() {
  ...
  try {
    ...
    mediaRecorder.addEventListener("stop", handleVoiceRecordingStop);
    mediaRecorder.start();
    isRecording = true;
...
async function handleVoiceRecordingStop() {
  try {
    ...
    voiceSupport.textContent = "转写完成，正在自动提交...";
    await submitAvailability(transcript.trim());
  } catch (error) {
    voiceSupport.textContent = `语音转写失败：${error.message || "请稍后再试"}`;
  } finally {
    cleanupRecorder();
  }
}
...
function cleanupRecorder() {
  if (mediaRecorder) {
    mediaRecorder.removeEventListener("stop", handleVoiceRecordingStop);
  }
  if (mediaStream) {
